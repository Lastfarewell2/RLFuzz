...
class DeepQNetwork(nn.Module):
    ...
    def choose_action(self, observation, percent, ori_size):
        ...
        if np.random.uniform() < self.epsilon:
            ...
            actions_value_np = actions_value.detach().numpy().argsort()[0]
            print(actions_value_np.size)
            actions = actions_value_np[- int(percent * actions_value_np.size) : ]
            print(actions.size)
        else:
            valid_actions = np.array([i for i in range(self.n_actions)])
            num_to_select = int(len(valid_actions) * percent)
            actions = np.random.choice(valid_actions, num_to_select, replace=False)
